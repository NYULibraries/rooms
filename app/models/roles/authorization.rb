module Roles
  module Authorization
    ...
    def auth_roles
      all_auth_roles.reject {|k,v| !v.include? attrs[:status] }.keys.map {|k| k.to_s}
    end
    ...
    def is_authorized?
      @is_authorized ||= Rails.cache.fetch "#{attrs[:status]}_authorized", :expires_in => 24.hours do
        all_auth_roles.delete_if {|key,value| !value.include? attrs[:status] }.blank?
      end
    end

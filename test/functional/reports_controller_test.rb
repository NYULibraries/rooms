...
class ReportsControllerTest < ActionController::TestCase
  ...
  test "generate basic report in CSV" do
    get :index, :report => { start_dt: (@reservation.start_dt.beginning_of_hour).strftime("%F %H:%M"), end_dt: (@reservation.end_dt.end_of_hour+1.minute).strftime("%F %H:%M") }, :format => :csv
    assert assigns(:reservations)
    assert assigns(:start_dt)
    assert assigns(:end_dt)
